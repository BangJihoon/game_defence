...
class OverflowDefenseGame extends FlameGame with HasCollisionDetection {
  ...
  Future<void> onLoad() async {
    ...
    for (var id in equippedIds) {
      ...
      final pc =
          ownedMap[id] ??
          PlayerCharacter(characterId: id, rank: charDef.startingRank);
    ...
    // 공격력은 보정된 값들의 평균 사용
    final avgAttack = equippedIds.isNotEmpty
        ? (totalWeightedAttack / equippedIds.length)
        : 0.0;

    // 밸런스 패치: 초반 공격력을 0.3배로 낮추어 성장하는 맛을 강화
    const double attackBalanceMultiplier = 0.3;
    final totalAttackPower =
        (avgAttack * attackBalanceMultiplier +
        playerDataManager.totalAttackPower);

    playerBase =
        PlayerBase(hp: totalBaseHp.toInt(), height: gameStats.baseSize.height)
    ...
    add(
      ItemSlotUI(
        equippedItemIds: playerDataManager.equippedToolIds,
        availableItems: playerDataManager.mysticTools,
      ),
    );
  ...
  void _subscribeToEvents() {
    eventBus.on<GameScoreChangedEvent>(
      (event) => scoreDisplay.updateScore(event.newGameScore),
    );
    ...
    eventBus.on<SkillVariantAppliedEvent>(
      (event) => skillSystem.applyVariant(event.skillId, event.variantId),
    );
    eventBus.on<LevelUpSkillEvent>(
      (event) => skillSystem.upgradeSkill(event.skillId),
    );
    eventBus.on<WallHealedEvent>((event) {
      final healAmount = playerBase.maxHp * event.amount;
      playerBase.hp = min(
        playerBase.maxHp.toDouble(),
        playerBase.hp + healAmount,
      );
    ...
    eventBus.on<WaveClearedEvent>((event) {
      ...
      if (event.waveNumber % 10 == 0)
        _showSpecialOracleOffer();
      else
  ...
  void _gameOver() {
    ...
    add(
      GameOverOverlay(
        gameScore: gameScore,
        waveReached: waveManager.currentWaveNumber,
  ...
  void showMessage(String message) {
    ...
    add(
      TimerComponent(
        period: 2.0,
        onTick: () => textComponent.removeFromParent(),
        removeOnFinish: true,
      ),
    );
  }
  ...
  void playBaseHitSound() {
    if (_soundEnabled) FlameAudio.play('base_hit.mp3', volume: 0.5);
  }

  void playEnemyDeathSound() {
    if (_soundEnabled) FlameAudio.play('enemy_death.mp3', volume: 0.2);
  }

  void playExplosionSound() {
    if (_soundEnabled) FlameAudio.play('explosion.mp3', volume: 0.4);
  }

  void playGameOverSound() {
    if (_soundEnabled) FlameAudio.play('game_over.mp3', volume: 0.6);
  }
...
class ItemSlotUI extends PositionComponent
    with HasGameRef<OverflowDefenseGame> {
  final List<String> equippedItemIds;
  final List<GameItem> availableItems;
  ItemSlotUI({required this.equippedItemIds, required this.availableItems})
    : super(priority: 100);
  @override
  Future<void> onLoad() async {
    super.onLoad();
      ...
      add(
        ItemButtonComponent(
          ...
          position: Vector2(
            0,
            (i * (btnSize + spacing)) -
                ((btnSize + spacing) * equippedItemIds.length / 2) +
                (btnSize / 2),
          ),
        ),
      );
...
class ItemButtonComponent extends PositionComponent
    with TapCallbacks, HasGameRef<OverflowDefenseGame> {
  final GameItem item;
  int charges = 1;
  ItemButtonComponent({required this.item, required Vector2 position})
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawRRect(
      ...
      Paint()..color = item.color.withValues(alpha: 0.8),
    );
  ...
  void onTapDown(TapDownEvent event) {
    if (charges > 0) {
      ...
      if (item.id == 'tsunami')
        _applyTsunami();
      else if (item.id == 'meteor_shower')
        _applyMeteorShower();
      else
        gameRef.enemySystem.damageInRadius(gameRef.size / 2, 2000, 200);
  ...
  void _triggerVisualEffects() {
    ...
    final flash = RectangleComponent(
      size: gameRef.size,
      paint: Paint()..color = item.color.withValues(alpha: 0.3),
    );
    gameRef.add(flash);
    flash.add(
  ...
  void _applyMeteorShower() {
    for (int i = 0; i < 10; i++) {
      final randomPos = Vector2(
        ...
        gameRef.random.nextDouble() * gameRef.size.y * 0.7,
      );
...
/// 상단 네비게이션 버튼 (홈, 설정용)
class TopNavButton extends PositionComponent
    with TapCallbacks, HasGameRef<OverflowDefenseGame> {
  final IconData icon;
